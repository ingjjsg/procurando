<?php
    ...
    function verOtrasFases($id){
        ...
        if ($id)
        {
            $cod=clConstantesModelo::otras_fases_litigio;
            if($id==$cod){
    ...
    function buscarExpedienteAuxiliar($str=""){
        $respuesta=new xajaxResponse();
...
//        $respuesta->script("$('contenedorAbogados').show();");            
    ...
    function selectAllExpedientesFiltro($pagina,$nro_tribunal="",$cedula_abogado_ejecutor="",$strexpediente="") {
        $respuesta= new xajaxResponse();
        $clientes= new clActuaciones();
        $data= "";
        $html= ""; 
        $dataC= $clientes->SelectAllExpedientesFiltro($pagina,$nro_tribunal,$cedula_abogado_ejecutor,$strexpediente);
        $data= $dataC[0];        
        if($data){
//            exit(print_r($data));
            $asistido=new clProContrarios();
