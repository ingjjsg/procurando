...
class clContactoModelo {
    ...
    public function updateContacto2(){
        $conn= new Conexion();
        $conn->abrirConexion();
        $sql= "UPDATE ".clConstantesModelo::seguridad_table."".clConstantesModelo::seguridad_table."tblcontacto SET ";
        $sql.= "strlogin= '".$this->getStrlogin()."', ";
        $sql.= "strtlfhab= '".$this->getStrtlfhab()."', strext='".$this->getStrext()."', stremail='".$this->getStremail()."', ";
        $sql.= "memdireccion= '".$this->getMemdireccion()."', ";
        $sql.= " strfirma='".$this->getStrfirma()."', ";
        $sql.= "strmediafirma= '".$this->getStrmediafirma()."' ";
        $sql.= "WHERE id_contacto=".$this->getId_contacto();
//        exit($sql);
        $conn->sql=$sql;
        $retorno= $conn->ejecutarSentencia();
        $conn->cerrarConexion();
    ...
    public function verificarIngreso(){
        $conn= new Conexion();
        $conn->abrirConexion();
        $login=clFunciones::reemplazarInjection($this->getStrlogin());
        $password=clFunciones::reemplazarInjection($this->getStrpassword());        
//        echo "SELECT * FROM ".clConstantesModelo::seguridad_table."tblcontacto WHERE strlogin='".$this->getStrlogin()."' AND strpassword= MD5('".$this->getStrpassword()."') AND id_estatus_maestro= 168 AND bolborrado= 0";
        $sql= "SELECT * FROM ".clConstantesModelo::seguridad_table."tblcontacto WHERE strlogin='".$login."' AND strpassword= MD5('".$password."') AND id_estatus_maestro= 168 AND bolborrado= 0";
//        exit($sql);
        $conn->sql= $sql;       
        $data= $conn->ejecutarSentencia(2);
        $conn->cerrarConexion();
...
//        exit($sql);
        ...
        public function insertarBitacora($id_expediente,$id_usuario,$id_tipo_accion,$id_modulo,$fecacaccion)
        {
//            exit('paso');
            $conn= new Conexion();
            $conn->abrirConexion();
            $sql="Insert into ".clConstantesModelo::correspondencia_table."tblbitacora (id_expediente,id_usuario,id_tipo_accion,id_modulo,fecacaccion)  
            VALUES ("
                    .$id_expediente.","
                    .$id_usuario.",'"
                    .$id_tipo_accion."','"
                    .$id_modulo."',TO_DATE('"
                    .$fecacaccion."', 'DD/MM/YYYY'))";
//            exit($sql);
            $conn->sql=$sql;
            $conn->ejecutarSentencia();
        }
