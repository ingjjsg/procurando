...
private   $strdescripcionestrategia;    
...
public function llenar($request)
{
     ...
     if($request['strdescripcionestrategia'] != ""){
        $this->strdescripcionestrategia= $request['strdescripcionestrategia'];
     }     
...
//exit('pasi');     
    ...
    public function getStrdescripcionestrategia(){
        return $this->strdescripcionestrategia;
    }    
...
//=========================== SET ===================
    ...
    public function setStrdescripcionestrategia($strdescripcionestrategia){
        return $this->strdescripcionestrategia=$strdescripcionestrategia;
    }
...
//================================FUNCION INSERTAR============================================
    ...
    static public function getExpediente($id) {
        $conn= new Conexion();
        $conn->abrirConexion();
        $sql = "select strnroexpediente from ".clConstantesModelo::correspondencia_table . "tblactuaciones  where id_proactuacion='".$id."'";        
        $conn->sql= $sql;
    ...
    public function SelectExpedienteDocumentoBuscar($id){
        ...
        $conn->abrirConexion();
        $sql="SELECT 
              id_proactuacion as id,
              strdescripcion,
              strnroexpediente
               FROM ".clConstantesModelo::correspondencia_table."tblactuaciones b WHERE b.id_proactuacion=".$id;        
//        exit($sql);
        $conn->sql=$sql;
        $data = $conn->ejecutarSentencia(2);
        return $data;    
        ...
        $sql="SELECT 
              id_proactuacion as id,
              (SELECT stritema FROM public.tblmaestros A WHERE A.id_maestro=b.id_origen LIMIT 1) AS id_origen_nombre,
              (SELECT stritema FROM public.tblmaestros A WHERE A.id_maestro=b.id_motivo LIMIT 1) AS id_motivo_nombre,
              strnroexpediente,
              (SELECT (upper(strapellido) || ', '::text) || upper(strnombre) FROM public.tbl_abogados A WHERE A.id_abogado=id_abogado_resp LIMIT 1) AS nombre_responsable
               FROM ".clConstantesModelo::correspondencia_table."tblactuaciones b WHERE b.strnroexpediente like '%".$expediente."%' limit 10";        
//        exit($sql);
        $conn->sql=$sql;
        $data = $conn->ejecutarSentencia(2);
        return $data;        
...
//         $this->actualizarDemandante($this->get_id_demandante());
         ...
         $sql="UPDATE ".clConstantesModelo::correspondencia_table.self::TABLA." SET
            ...
            strdescripcionestrategia='".$this->getStrdescripcionestrategia()."',                
...
//         exit($sql);
     ...
     public function selectGrafica_abogados_casos_cargados_total($fecini,$fecfin){
        ...
        $conn->abrirConexion();
        $sql="SELECT
                count(tblactuaciones.id_abogado_resp) AS contador, 
                (SELECT (upper(tbl_abogados.strapellido) || ', '::text) || upper(tbl_abogados.strnombre) FROM  tbl_abogados WHERE tbl_abogados.id_abogado = tblactuaciones.id_abogado_resp) AS strnombre
                FROM 
                tblactuaciones
                WHERE 
                tblactuaciones.feccierre IS NULL and (fecapertura >='".$fecini."' and fecapertura <='".$fecfin."' )
                GROUP BY 
                tblactuaciones.id_abogado_resp
                ORDER BY ( SELECT (upper(tbl_abogados.strapellido) || ', '::text) || upper(tbl_abogados.strnombre)
                FROM 
                tbl_abogados
                WHERE 
                tbl_abogados.id_abogado= tblactuaciones.id_abogado_resp ), 
                count(tblactuaciones.id_abogado_resp ) DESC";
//        exit ($sql);
        $conn->sql= $sql;
        $retorno= $conn->ejecutarSentencia(2);
        $conn->cerrarConexion();
    ...
    public static function getExpedienteClienteAgendaLitigio($id_cliente){
        $conn= new Conexion();
        $conn->abrirConexion();
        $sql="SELECT 
            id_proactuacion,
            ...
            to_char(fecapertura,'DD/MM/YYYY') as fecapertura,
            id_tipo_organismo_centralizado
            FROM ".clConstantesModelo::correspondencia_table."tblactuaciones WHERE id_proactuacion=".$id_cliente;        
//        exit($sql);
        $conn->sql= $sql;
        $data= $conn->ejecutarSentencia(2);
        $conn->cerrarConexion();
