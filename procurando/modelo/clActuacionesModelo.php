...
//         exit('hola');
         ...
         $sql.="TO_DATE('".$this->getFecapertura()."', 'DD/MM/YYYY'),";         
         $sql.="TO_DATE('".$this->getFecexpediente()."', 'DD/MM/YYYY'),";       
...
a;
    ...
    public function SelectAllExpedientesFiltro($pagina,$nro_tribunal="",$cedula_abogado_ejecutor="",$strexpediente=""){
         $conn= new Conexion();
         $conn->abrirConexion();
         ...
         FROM ".clConstantesModelo::correspondencia_table."tblactuaciones a
             WHERE 
             a.bolborrado=0 and";
         if (($_SESSION['id_profile']!=  clConstantesModelo::administrador_sistema) || ($_SESSION['id_profile']!=clConstantesModelo::coordinador_sistema))
              $sql.=" a.id_usuario=".$_SESSION['id_contacto'];

         if($nro_tribunal != ""){
             $sql .= " AND a.strnroexpedienteauxiliar LIKE '%$nro_tribunal%'";
         }
         if($cedula_abogado_ejecutor != ""){
             $sql .= " AND d.strcedula LIKE '%$cedula_abogado_ejecutor%'";
         }
         if($strexpediente != ""){
...
//exit($sql);
    ...
    public function eliminarDemandanteReferido($lngcodigo){
     ...
     public function SelectAllExpedienteReporte($id_tipo_tramite,$id_tipo_atencion,$id_actuacion_persona,$id_tipo_organismo,$id_organismo,$id_tipo_fase,$id_fase,$strnroexpediente,$strnroexpedienteauxiliar){
         ...
         left join public.tblactuacion_fases fases using(id_proactuacion) WHERE actuacion.bolborrado=0";
