...
//        exit($sql);        
    ...
    public function selectAllMaestroHijos($padre, $campo, $id_sistemas="", $lngnumero=0){
        ...
        if ($padre>0) $sql_insert_padre=" AND a.id_origen= ".$padre;                
        $sql="SELECT a.*, b.stritema as sistema FROM ".clConstantesModelo::correspondencia_table."tblmaestros_vista a, ".clConstantesModelo::correspondencia_table."tblmaestros_sistemas b WHERE a.id_sistema=b.id_sistema ".$sql_insert_padre." ".$sql_insert." ".$sql_insert_hijo." AND a.bolborrado= 0 ORDER BY a.".$campo." ".$_SESSION["AD"];
//        exit($sql);        
        $conn->sql= $sql;
        $data= $conn->ejecutarSentencia(2);
        $conn->cerrarConexion();
