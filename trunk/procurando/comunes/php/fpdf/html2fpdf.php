...
function HTML2FPDF($orientation='P',$unit='mm',$format='A4', $codigoBarra= '', $usuario= "", $fechaEnvio= "")
{
//! @desc Constructor
//! @return An object (a class instance)
  ...
  $this->DisplayPreferences('');
	...
	$this->SetFont('Arial','',8); // Changeable?(not yet...)
...
//! @desc The header is printed in every page.
 ...
 // }//end of 'if usetableheader ...'
   //Logo
    $this->Image('../comunes/images/cab_reportes.jpg',10,2,190);
	 $this->Ln(5);
    //Arial bold 15
...
//! @desc The footer is printed in every page!
	...
	//Logo
		if($this->codigoBarra != "" && $this->usuario != "" && $this->fechaEnvio != ""){
			$this->SetFont('Arial', 'BI', 5);		
			$this->Code39(65,248, $this->codigoBarra, 0.5, 3);
			$this->SetFont('Arial', 'BI', 6);
			$this->Text(19, 256, utf8_decode("La presente correspondencia ha sido enviada por ".$this->usuario." a través del sistema de correspondencia en fecha ".$this->fechaEnvio.", para confirmar la emisión de este"));
			$this->Text(35, 259, utf8_decode("documento comuníquese a la Oficina de Tecnologías de la Información a los telf: (0212)576.11.50/(0212)578.27.34 ext. 1017, 1018, 1011, 1035."));
		}
		$this->Image('../comunes/images/foot_reportes.jpg',10,260,190);
...
//! @desc Prints all list-related buffered info
    ...
    foreach($this->listitem as $item)
    {
        ...
        if ($type == chr(110)) $this->SetFont('zapfdingbats','',6);
        $type .= ' ';
			  ...
			  case 'FONT-SIZE': //Does not support: smaller, larger
			      if(is_numeric($v{0}))
			      {
  			      ...
  			      switch($v)
  			      {
  			         ...
  			         case 'XX-SMALL': $this->SetFontSize( (0.7)* 8);
  			             break;
                 case 'X-SMALL': $this->SetFontSize( (0.77) * 8);
			               break;
			           case 'SMALL': $this->SetFontSize( (0.86)* 8);
  			             break;
  			         case 'MEDIUM': $this->SetFontSize(8);
  			             break;
  			         case 'LARGE': $this->SetFontSize( (1.2)*8);
  			             break;
  			         case 'X-LARGE': $this->SetFontSize( (1.5)*8);
  			             break;
  			         case 'XX-LARGE': $this->SetFontSize( 2*8);
			               break;
              }
            }
			   	  break;
