...
//        exit(print_r($data));
        ...
        if ($data[0][id_expediente]>0)
        {
            $litigio=clActuaciones::getExpediente($data[0][id_expediente]);
            $oas=clProExpediente::getExpediente($data[0][id_expediente]);

            if ($litigio!='')
                $respuesta->assign('strnroexpediente', 'value', $litigio);                   
            else if ($oas!='')
                $respuesta->assign('strnroexpediente', 'value', $oas);                      
             $respuesta->script("$('CapaExpediente').show();");             
        }
        if ($data[0][id_contacto]>0)
        {
...
//        exit(print_r($data));        
    ...
    function buscarExpediente($id,$oficina){
        $respuesta=new xajaxResponse();
        $expedienteOAS = new clProExpediente();  
        $expedienteLTG = new clActuaciones();   
        if ($oficina=='LTG')
        {
            $data=$expedienteLTG->SelectExpedienteDocumentoBuscar($id);
        }    
        elseif  ($oficina=='OAS')
        {
            $data=$expedienteOAS->SelectExpedienteDocumentoBuscar($id);
        }          
        if(is_array($data)){
            $respuesta->assign("strnroexpediente", "value", $data[0][strnroexpediente]);
            $respuesta->assign("id_proexpediente", "value", $data[0][id]);  
...
//        exit($str."PASO");
        ...
        $data= "";
        $html= "";  
        $oficina= strtoupper(substr($str, 0,3));
        if ($oficina=='LTG')
        {
            $data=$expedienteLTG->SelectExpedienteDocumento($str);
            $ofiurl='LTG';
        }    
        elseif  ($oficina=='OAS')
        {
            $data=$expedienteOAS->SelectExpedienteDocumento($str);
            $ofiurl='OAS';            
        }              
        if($data){
                $html= "<div style='border:solid 1px #CCCCCC;background:#f8f8f8'>
                ...
                for ($i= 0; $i < count($data); $i++){
                    ...
                    $html.= "<tr bgcolor='#f8f8f8'onmouseover=\"this.style.background='#f0f0f0';this.style.color='blue'\" onmouseout=\"this.style.background='#f8f8f8';this.style.color='black'\" >
                                ...
                                <td align='center'>".$data[$i]['id_origen_nombre']."</td>
                                <td align='center'>".strtoupper($data[$i]['id_motivo_nombre'])."</td>                       
                                <td align='center'>
                                        <a>
                                            <img src='../comunes/images/s_success.png' onmouseover='Tip(\"Elegir Expediente\")' onmouseout='UnTip()' onclick=\"xajax_buscarExpediente('".$data[$i][id]."','".$ofiurl."')\">
                                        </a>                                   
                                </td>
                            </tr>";
                }
...
//            exit($html);
            for ($i= 0; $i < count($data); $i++){
                ...
                $html.= "<td width='5%' align='center'><img src='../comunes/images/".$data[$i][origen].".png' height='20px' onmouseover=\"Tip('".clTblDocumento::getMovimientoDocumentos($movimiento)."', TITLE, 'Ruta del Documento ".$data[$i][date]."')\" onmouseout='UnTip()'\"></td>
                            ...
                            <td width='35%' align='center'>";
//                                      exit('aqui2');
                $color=functions::diterenciaFechasSemaforoAgenda($data[$i]['fecdocumento'],clTblDocumento::getDiasRecordatorios($data[$i]['id_recordatorio']));
