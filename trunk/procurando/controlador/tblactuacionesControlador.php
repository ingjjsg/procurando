<?php
    ...
    function Resetear_busqueda(){
        $respuesta=new xajaxResponse();
            $_SESSION['nro_tribunal_reporte']='';             
            $_SESSION['cedula_abogado_ejecutor_reporte']='';          
            $_SESSION['strexpediente_reporte']='';          
            $_SESSION['id_responsable_reporte']='';          
            $_SESSION['id_origen_reporte']='';          
            $_SESSION['id_motivo_reporte']='';            
...
//        exit(print_r($data));
    ...
    function GraficosTotales($fecini='',$fecfin='') {
        ...
        $html.= '<h2>Desempeño Integrantes por Departamento</h2>';
        $chart->setChartAttrs( array(
                'type' => 'pie',
                'title' => 'OFICINA DE LITIGIO ',
                'data' => $data,
                'size' => array( 500, 300 ),
                'color' => $color,
                'labelsXY' => true
...
//        exit('paso');
        ...
        if ($id_nuevo_abogado!='')
        {
            ...
            if($data){
                ...
                $contacto->insertarBitacora($request[id_proactuacion], $_SESSION['id_contacto'], 'R', 'LTG', date('d/m/Y'));
          ...
          $respuesta->alert("Nuevo Abogado no esta Registrado como Abogado, solo como Usuario del Sistema");            
        return $respuesta;
    }    
...
//        $respuesta->script("$('contenedorAbogados').show();");            
        ...
        if (clTblproexpediente_personas_demandadas::getBuscarPersonaDemandadasExpediente($id_contrario,$id_expediente)=='')
        {
            if (($cedula!='') and ($id_expediente!=''))
            {
                ...
                if($expediente_contrarios->insertar($id_contrario,$id_expediente)){
                    ...
                    $contacto->insertarBitacora($id_expediente, $_SESSION['id_contacto'], 'IPD', 'LTG-ID_ABOGADO='.$id_contrario, date('d/m/Y'));
    ...
    function Eliminar_Abogados_Expediente($id, $id_expediente, $tipo){
        ...
        if ($tipo=='E')
        {
            ...
            $contacto->insertarBitacora($id_expediente, $_SESSION['id_contacto'], 'D', 'LTG-EJECUTOR:'.$id, date('d/m/Y'));
        ...
        else if ($tipo=='D')
        {
            ...
            $contacto->insertarBitacora($id_expediente, $_SESSION['id_contacto'], 'D', 'LTG-DEMANDANTE:'.$id, date('d/m/Y'));
        ...
        else if ($tipo=='R')
        {
            ...
            $contacto->insertarBitacora($id_expediente, $_SESSION['id_contacto'], 'D', 'LTG-REPRESENTANTE:'.$id, date('d/m/Y'));
        ...
        else if ($tipo=='P')
        {
            ...
            $index->delete($id, $id_expediente);
            $respuesta->script("xajax_ListaPersonasDemandadas('".$id_expediente."')");            
            $contacto->insertarBitacora($id_expediente, $_SESSION['id_contacto'], 'D', 'LTG-PERSONA:'.$id, date('d/m/Y'));
        ...
        if (clTblproexpediente_abogados_representantes::getBuscarAbogadoExpediente($id_abogado,$id_expediente)=='')
        {
            if (($cedula!='') and ($id_expediente!=''))
            {
                ...
                if($expediente_abogados->insertar($id_abogado,$id_expediente)){
                    $respuesta->script("xajax_ListaAbogadosRepresentantes('".$id_expediente."')");
                    $contacto->insertarBitacora($id_expediente, $_SESSION['id_contacto'], 'IAR', 'LTG-ID_ABOGADO='.$id_abogado, date('d/m/Y'));
    ...
    function IngresarAbogadoExpedienteDemandantes($cedula,$id_expediente,$tipo){
        ...
        if (clTblproexpediente_abogados_demandantes::getBuscarAbogadoExpediente($id_abogado,$id_expediente)=='')
        {
            if (($cedula!='') and ($id_expediente!=''))
            {
                ...
                if($expediente_abogados->insertar($id_abogado,$id_expediente)){
                    $respuesta->script("xajax_ListaAbogadosDemandantes('".$id_expediente."')");
                    $contacto->insertarBitacora($id_expediente, $_SESSION['id_contacto'], 'IAD', 'LTG-ID_ABOGADO='.$id_abogado, date('d/m/Y'));
    ...
    function IngresarAbogadoEjecutorExpediente($cedula,$id_expediente,$tipo){
        ...
        if (clTblproexpediente_abogados_ejecutores::getBuscarAbogadoExpediente($id_abogado,$id_expediente)=='')
        {
            if (($cedula!='') and ($id_expediente!=''))
            {
                ...
                if($expediente_abogados->insertar($id_abogado,$id_expediente)){
                    $respuesta->script("xajax_ListaAbogadosEjecutores('".$id_expediente."')");
                    $contacto->insertarBitacora($id_expediente, $_SESSION['id_contacto'], 'IAE', 'LTG-ID_ABOGADO='.$id_abogado, date('d/m/Y'));
...
function selectAllActuaciones($id_expediente){
    ...
    function eliminar_actuacion($id_litigio_actuaciones,$id_proactuacion){
        ...
        $data = $actuacion->Delete($id_litigio_actuaciones);
             if($data){
                $contacto->insertarBitacora($id_proactuacion, $_SESSION['id_contacto'], 'DAC', 'LTG', date('d/m/Y'));
	...
	$actuaciones= new cltblproactuaciones_litigio();
        ...
        {
            $contacto->insertarBitacora($request[id_proactuacion], $_SESSION['id_contacto'], 'IAC', 'LTG', date('d/m/Y'));
...
//                }
                $html.= "<tr bgcolor='#f8f8f8'onmouseover=\"this.style.background='#f0f0f0';this.style.color='blue'\" onmouseout=\"this.style.background='#f8f8f8';this.style.color='black'\" >
                            <td align='center'><a target=\"contenido\" href=\"vista_Ingresotblexpediente.php?id=".$data[$i]['id_proactuacion']."\">".$data[$i]['strnroexpediente']."</a></td>                        
...
//        exit("Voy");          
            ...
            if($data){
                ...
                $contacto->insertarBitacora($id, $_SESSION['id_contacto'], 'I', 'LIT', date('d/m/Y'));
            }else{
...
//            }
        ...
        $respuesta->assign('id_proactuacion', 'value', $data[0]['id_proactuacion']);
            ...
            $scr = "../vista/vista_tblagenda_Litigio.php?id=" . $data[0]['id_proactuacion'];            
            $html='<iframe  width="100%" scrolling="auto" height="800" frameborder="0" src="'.$scr.'" scrolling="auto" frameborder="0" width="100%" height="600"></iframe>';
            $respuesta->assign("capaIdTipoAgendaExpediente","innerHTML",$html);               
...
//        $respuesta->script("FCKeditorAPI.__Instances['letrado'].SetHTML('".$data[0][strletrado]."')");        
        $respuesta->script("FCKeditorAPI.__Instances['descripcionest'].SetHTML('".$data[0][strdescripcionestrategia]."')");
...
//            $data_demandante= $cliente->actualizarDemandante();
            if($data){
                ...
                $contacto->insertarBitacora($request[id_proactuacion], $_SESSION['id_contacto'], 'U', 'LTG', date('d/m/Y'));
        ...
        $contacto= new clContactoModelo();        
            $data = $expediente->Delete($id_expediente);
             if($data){
                ...
                $contacto->insertarBitacora($id_expediente, $_SESSION['id_contacto'], 'D', 'LTG', date('d/m/Y'));
                $respuesta->script("xajax_buscarDatosExpedientes()");
            }else{
