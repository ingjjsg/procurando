<?php
    session_start();
    require_once '../modelo/clAdjuntarDocumentoModelo.php';
 ...
 */
    ...
    function eliminar_documento($id_expediente="",$id_file=""){
        ...
        $file = new clAdjuntarModeloExpediente();
        $contacto= new clContactoModelo();        
        
            $data = $file->deleteAdjuntoById($id_file);
             if($data)
             {
                $contacto->insertarBitacora($id_expediente, $_SESSION['id_contacto'], 'DFI', 'LTG-D:'.$id_file, date('d/m/Y'));
                $respuesta->alert("Ha sido Eliminado el Archivo del Expediente con Exito");
                $respuesta->script("xajax_ListaDocumentosExpediente('".$id_expediente."')");
            }else{
                $respuesta->alert("El Archivo del Expediente no se ha Eliminado");
            }
        return $respuesta;
    }     
...
//        exit($id_expediente);
        ...
        $expediente= new clAdjuntarModeloExpediente();
        $data= "";
        $html= "";         
                            ...
                            <tr>
                                <td colspan=\"5\" align=\"right\" style=\"color:white; border:#CCCCCC solid 0px;\" bgcolor=\"#273785\" >
                                    <div align=\"center\" style=\"background-image: url('../comunes/images/barra.png')\">
                                        <strong>LISTADO DE DOCUMENTOS ANEXADOS AL EXPEDIENTE</strong>
                                    </div>                                
                                </td>
                            </tr>                             
                                <tr>
...
//                    exit(print_r($file));
                    $html.= "<tr bgcolor='#f8f8f8'onmouseover=\"this.style.background='#f0f0f0';this.style.color='blue'\" onmouseout=\"this.style.background='#f8f8f8';this.style.color='black'\" >
                                ...
                                <td align='center'>
                                        <a>
                                            <input type='images' value='Eliminar Archivo' id='boton' name='boton' style='background-color:#B9D5E3;border:1px outset #B9D5E3;color:#004E7D;cursor:pointer;margin:1px;padding:1px; width:100px; font-size:11px;' onclick=\"xajax_eliminar_documento('".$data[$i]['id_proexpediente']."','".$data[$i]['id_adjunto']."','P')\">
                                       </a>                                   
                                </td>
                            </tr>";
                }
...
class adjuntarDocumentoControlador {
    ...
    function insertAdjunto($id_archivo, $stradjunto) {
        $adjunto= new clAdjuntarModeloExpediente();
    ...
    function selectAdjunto($id_archivo){
		...
		if($data){
            ...
            foreach($data as $adjunto){
                ...
                echo "&nbsp;&nbsp;<a href='bajarAdjuntoVista.php?f=../comunes/uploads/documentos/".$adjunto["id_archivo"]."_".$adjunto["stradjunto"]."'>".$adjunto["stradjunto"]."</a>";
                echo "&nbsp;&nbsp;&nbsp;<a title='Eliminar' href='adjuntarDocumentoVista.php?adjunto=".$adjunto["id_archivo"]."&eliminar=".$adjunto["id_adjunto"]."'><strong>[X]</strong></a><br>";
            }
		}else{
            echo "";
		}
    ...
    function deleteAdjunto($id_ajunto){
        ...
        if($data){
            unlink("../comunes/uploads/documentos/".$data[0]["id_archivo"]."_".$data[0]["stradjunto"]);
        }else{
            echo "";
        }
    }
