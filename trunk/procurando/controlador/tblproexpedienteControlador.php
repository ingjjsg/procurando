<?php
    session_start();
    require_once '../comunes/grafic/GoogChart.class.php';    
    ...
    function Resetear_busqueda(){
        $respuesta=new xajaxResponse();
            $_SESSION['cedula_cliente_reporte']="";
            $_SESSION['cedula_abogado_responsable_reporte']="";        
            $_SESSION['cedula_abogado_ejecutor_reporte']="";        
            $_SESSION['strexpediente_reporte']="";                
//        $respuesta->alert("Busqueda Inicializada");   
        return $respuesta;
    }    
    
    function buscarExpedienteVista($expediente_buscado=''){
        $respuesta=new xajaxResponse();
        $expediente= new clProExpediente();
        if ($expediente_buscado!='')
        {
                $expediente_buscado=clProExpediente::getExpedienteVista(trim($expediente_buscado));
                if($expediente_buscado){
                    $respuesta->assign('strnroexpediente', 'value', 'OAS-' . date('dmY') . '-##');
                    $respuesta->alert("El Numero de Expediente ya Existe");
                }
        }
        return $respuesta;
    }      
    
    function selectLeyendaGrafico_abogados_casos_cargados_total($fecini='',$fecfin='') {
        if ($fecini=='') $fecini=date('Y-m-d');  else $fecini= substr($fecini,6,4).'-'.substr($fecini,3,2).'-'.substr($fecini,0,2);
        if ($fecfin=='') $fecfin=date('Y-m-d');  else $fecfin= substr($fecfin,6,4).'-'.substr($fecfin,3,2).'-'.substr($fecfin,0,2);     
        $respuesta= new xajaxResponse();
        $expediente= new clProExpediente();
        $data= "";
        $html= ""; 
        $total=0;
        $data= $expediente->selectGrafica_abogados_casos_cargados_total($fecini,$fecfin);
//        exit(print_r($data));
        if($data){
    ...
    function GraficosTotales($fecini='',$fecfin='') {
        if ($fecini=='') $fecini=date('Y-m-d');  else $fecini= substr($fecini,6,4).'-'.substr($fecini,3,2).'-'.substr($fecini,0,2);
        if ($fecfin=='') $fecfin=date('Y-m-d');  else $fecfin= substr($fecfin,6,4).'-'.substr($fecfin,3,2).'-'.substr($fecfin,0,2);     
        $chart = new GoogChart();        
        $respuesta= new xajaxResponse();
        ...
        $data = array();
        $html= ""; 
        $data_arreglo= $expediente->selectGrafica_abogados_casos_cargados_total($fecini,$fecfin);
        if($data_arreglo){
            foreach ($data_arreglo as $row) {
                $data += array($row['strnombre']=> $row['contador']);
            }   
        }
        $color = array('#99C754','#54C7C5','#999999',);
        $html.= '<h2>Expedientes Abiertos</h2>';
        $chart->setChartAttrs( array(
                'type' => 'pie',
                'title' => 'OFICINA DE ATENCION AL CIUDADANO',
                'data' => $data,
                'size' => array( 500, 300 ),
                'color' => $color,
                'labelsXY' => true
        ...
        $data2 = array();
        $data_arreglo_cerrados= $expediente->selectGrafica_abogados_casos_cargados_total_cerrados($fecini,$fecfin);
        if($data_arreglo_cerrados){
            foreach ($data_arreglo_cerrados as $row) {
                $data2 += array($row['strnombre']=> $row['contador']);
            }   
        }
        
        
        $html.= '<h2>Expedientes Cerrados</h2>';
        $chart->setChartAttrs( array(
                'type' => 'bar-vertical',
                'title' => 'OFICINA DE ATENCION AL CIUDADANO',
                'data' => $data2,
...
//        exit($id_nuevo_abogado);
        if ($id_nuevo_abogado!='')
        {        
            $data= $expediente->update($cedula,$id_nuevo_abogado);
            if($data){
                $data= $expediente->insertar();
                $contacto->insertarBitacora($request[id_proexpediente], $_SESSION['id_contacto'], 'REA', 'OAS', date('d/m/Y'));
            ...
            $respuesta->alert("Abogado a reasignar no se encuentra como Abogado del Instituto, Verifique que exista en el formulario Abogados de la Procuraduria y verifique  sus datos");            
        return $respuesta;
    }    
...
//                exit('paso');
   ...
   function selectActuacionHijo($id_proactuaciones,$id_solicitante,$cedula_responsable,$nombre_responsable,$cedula_ejecutor,$nombre_ejecutor){
        $respuesta= new xajaxResponse();
	$actuaciones= new clTblproactuaciones();
        $data= "";
        $data= $actuaciones->selectDetalleActuacion($id_proactuaciones);
        if($data){
            $text = str_replace(clConstantesModelo::NOMBRESOLICITANTE,clProClientes::getNombreClienteExpediente($id_solicitante),$data[0]['strdescripcionactuacion']);
            $text = str_replace(clConstantesModelo::CEDULASOLICITANTE,clProClientes::getNombreCIClienteExpediente($id_solicitante),$text);            
            $text = str_replace(clConstantesModelo::CEDULAABOGADORESPONSABLE,strtoupper($cedula_responsable),$text);            
            $text = str_replace(clConstantesModelo::NOMBREABOGADORESPONSABLE,strtoupper($nombre_responsable),$text);                        
            $text = str_replace(clConstantesModelo::NOMBREABOGADOEJECUTOR,strtoupper($nombre_ejecutor),$text);            
            $text = str_replace(clConstantesModelo::CEDULAABOGADOEJECUTOR,strtoupper($cedula_ejecutor),$text);            
	    $respuesta->script("FCKeditorAPI.__Instances['descripcionact'].SetHTML('".$text."')");
        }
        return $respuesta;
    }
	...
	$actuaciones= new clTblproactuaciones();
        ...
        $html= "<select id='id_escrito' name='id_escrito' style='width:50%' onchange=\"xajax_selectActuacionHijo(document.frminscribir.id_escrito.value,document.frminscribir.id_solicitante.value,document.frminscribir.cedula_abogado_responsable.value,document.frminscribir.strnombre_abogado_responsable.value,document.frminscribir.cedula_abogado_ejecutor.value,document.frminscribir.strnombre_abogado_ejecutor.value);\">";
        $html.= "<option value='0'>Seleccione</option>";
        if($data){
            for ($i= 0; $i < count($data); $i++){
               $seleccionar= "";
    ...
    function buscarDatosExpedientes($pagina){
        ...
        if($data){
            ...
            for ($i= 0; $i < count($data); $i++){
                ...
                $html.= "<tr bgcolor='#f8f8f8'onmouseover=\"this.style.background='#f0f0f0';this.style.color='blue'\" onmouseout=\"this.style.background='#f8f8f8';this.style.color='black'\" >
                            <td align='center'><a target=\"contenido\" href=\"vista_Ingresotblexpediente.php?id=".$data[$i]['id_proexpediente']."\">".$data[$i]['strnroexpediente']."</a></td>                        
                            <td align='center'><a target=\"contenido\" href=\"vista_nuevo_cliente.php?id=".$data[$i]['id_solicitante']."\">".$data[$i]['cedula_cliente']."</a></td>                        
                            <td align='center'><a target=\"contenido\" href=\"vista_nuevo_cliente.php?id=".$data[$i]['id_solicitante']."\">".strtoupper($nombre_asistido)."</a></td>                        
...
//        exit($_SESSION['cedula_abogado_responsable_reporte']);
        $respuesta= new xajaxResponse();
        $clientes= new clProExpediente();
        if ($cedula_cliente!='')
        ...
        $respuesta->assign("cedula_cliente", "value", $_SESSION['cedula_cliente_reporte']);         
        $respuesta->assign("cedula_abogado_responsable", "value", $_SESSION['cedula_abogado_responsable_reporte']);         
        $respuesta->assign("cedula_abogado_ejecutor", "value", $_SESSION['cedula_abogado_ejecutor_reporte']);         
        $respuesta->assign("strexpediente", "value", $_SESSION['strexpediente_reporte']);         
        $respuesta->assign("contenedor","innerHTML",$html);
        $respuesta->assign("pag","innerHTML",$dataC[1]);               
        return $respuesta;
...
//                exit('paso');  
                ...
                if($data){
                    ...
                    $contacto->insertarBitacora($id, $_SESSION['id_contacto'], 'I', 'OAS', date('d/m/Y'));
    ...
    if ($data) {
        ...
        $respuesta->assign('strreferencia', 'value', strtoupper($data[0]['strreferencia']));
        $respuesta->assign('id_abogado_ejecutor', 'value', $data[0]['id_abogado_ejecutor']);        
        $respuesta->assign('id_cliente', 'value', $data[0]['id_cliente']);
        $respuesta->assign('id_contrarios', 'value', $data[0]['id_contrarios']); 
            $scr = "../vista/vista_tblagenda_Expediente.php?id=" . $data[0]['id_proexpediente'];            
...
//   exit(print_r($request));
        ...
        else
        {
            ...
            if($data){
                ...
                $contacto->insertarBitacora($request[id_proexpediente], $_SESSION['id_contacto'], 'U', 'OAS', date('d/m/Y'));
                if (($request[feccierre]!='') and  ($request[strobservacion_cerrar]!='')) 
                   $contacto->insertarBitacora($request[id_proexpediente], $_SESSION['id_contacto'], 'C', 'OAS', date('d/m/Y'));
        ...
        $contacto= new clContactoModelo();        
            $data = $expediente->Delete($id_expediente);
             if($data){
                ...
                $contacto->insertarBitacora($id_expediente, $_SESSION['id_contacto'], 'D', 'OAS', date('d/m/Y'));
                $respuesta->script("xajax_buscarDatosExpedientes()");
            }else{
