<?php
    ...
    $pdf->Cell(0, 4, utf8_decode("Observaciones de la Actividad ".$dataActividad[0]['memtitulo']." (Id: ".$dataActividad[0]['id_actividad'].")"), 0, 1, 'C');
    $pdf->Ln();
    $pdf->SetFont('Arial', 'BI', 12);
    ...
    if($dataContactoActividad[0]['id_contactoactividad'] != ""){
        for($i= 0; $i < count($dataContactoActividad); $i++){
            ...
            if($dataDetalleContactoActividad[0]['id_detallecontactoactividad'] != ""){
                ...
                $pdf->Row(array('Fecha y Hora', utf8_decode("Observaciones")));
                ...
                for($y= 0; $y < count($dataDetalleContactoActividad); $y++){
                    ...
                    $pdf->Row(array(utf8_decode($dataDetalleContactoActividad[$y]['fecha_registro']." | ".$dataDetalleContactoActividad[$y]['hora_registro']), utf8_decode($dataDetalleContactoActividad[$y]['menobservaciones'])));
                }
            }else{
                $pdf->SetFont('Arial', 'B', 10);
                $pdf->Cell(0, 4, utf8_decode("No Hay Observaciones"), 1, 1, 'L');
            }
            $pdf->Ln(5);
        }
    }
